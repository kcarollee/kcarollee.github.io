...
/*
  ...
  */
  ...
  float gd = map(mouse.x, .0, 1.0, .0, 5.0); // density
...
/*
  if (val > .0){
    val = sin(p.x * gd * 0.5 + dt) * cos(p.y * gd * 0.5 + dt) + 
  sin(p.y * gd * 0.5 + dt) * cos(p.z * gd * 0.5 + dt) + 
  sin(p.z * gd * 0.5 + dt) * cos(p.x * gd * 0.5 + dt);
  }
  else val = 5.0;
  */
  return val;
...
float GyroBox(vec3 p, vec3 pos, vec3 s){
  float g = Gyroid(p, pos, 1.0);
  float b = Box(p, pos, s);
  float f = 10000.0;
  f = min(b, f);
  f = max(g, f);
...
float gs1 = GyroBox(p, vec3(.0), vec3(4.0));
